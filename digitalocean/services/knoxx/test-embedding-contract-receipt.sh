...
write_embedding_contract_receipt \
  "$receipt_path" nomic-embed-text 768 "$fingerprint"
[ "$(stat -c '%a' "$receipt_path")" = 600 ] || fail "receipt mode is not 600"
read_embedding_contract_receipt "$receipt_path"
[ "$EMBED_RECEIPT_PRESENT" = 1 ] || fail "valid receipt was not loaded"
[ "$EMBED_RECEIPT_MODEL" = nomic-embed-text ] || fail "model did not round trip"
[ "$EMBED_RECEIPT_DIMENSIONS" = 768 ] || fail "dimensions did not round trip"
...
if write_embedding_contract_receipt "$receipt_path" 'bad model' 768 "$fingerprint"; then
...
jq -n \
  ...
  '{version: 1, model: "nomic-embed-text", dimensions: "768",
...
if write_embedding_contract_receipt \
    "$receipt_path" nomic-embed-text 768 "$fingerprint"; then
