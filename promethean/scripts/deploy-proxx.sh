...
: "${PROXX_SOURCE_ROOT:?Path to local open-hax/proxx checkout is required}"
...
case "$DEPLOY_ENV" in
  production)
    ...
    : "${DEPLOY_PATH:=~/devel/services/proxx-production}"
    : "${DEPLOY_COMPOSE_PROJECT_NAME:=proxx-production}"
    : "${DEPLOY_COMPOSE_FILES:=docker-compose.federation-runtime.yml,deploy/docker-compose.federation.no-ssl.yml}"
    : "${DEPLOY_CADDY_TEMPLATE:=deploy/Caddyfile.federation.template}"
    : "${DEPLOY_HEALTH_SERVICE:=federation-nginx}"
    : "${DEPLOY_RESTART_SERVICES:=federation-proxx-a1,federation-proxx-a2,federation-proxx-b1,federation-proxx-b2,federation-nginx}"
    : "${DEPLOY_PUBLIC_HOST:=proxx.promethean.rest}"
    : "${DEPLOY_ENABLE_TLS:=false}"
    : "${DEPLOY_HEALTH_TIMEOUT_SECONDS:=240}"
    : "${DEPLOY_PRUNE_BEFORE_BUILD:=false}"
    : "${DEPLOY_ENV_APPEND:=FEDERATION_CLUSTER_ID=prod
FEDERATION_PUBLIC_HOST_SUFFIX=${DEPLOY_PUBLIC_HOST}
FEDERATION_DEFAULT_OWNER_SUBJECT=did:web:${DEPLOY_PUBLIC_HOST}
FEDERATION_NGINX_PUBLISHED_PORT=8891
PROXX_CONTAINER_UID=1000
PROXX_CONTAINER_GID=1000
PROXX_RUNTIME_DATA_DIR=./runtime-data-v3
PROXX_CONTAINER_HOME=/home/node
...
XDG_CACHE_HOME=/home/node/.cache}"
  ...
  staging)
    : "${DEPLOY_HOST:=$PROMETHEAN_SSH_HOST}"
    : "${DEPLOY_USER:=$PROMETHEAN_SSH_USER}"
    : "${DEPLOY_PATH:=~/devel/services/proxx-staging}"
    : "${DEPLOY_COMPOSE_PROJECT_NAME:=proxx-staging}"
    : "${DEPLOY_COMPOSE_FILES:=docker-compose.federation-runtime.yml,deploy/docker-compose.federation.no-ssl.yml}"
    : "${DEPLOY_CADDY_TEMPLATE:=deploy/Caddyfile.federation.template}"
    : "${DEPLOY_HEALTH_SERVICE:=federation-nginx}"
    : "${DEPLOY_RESTART_SERVICES:=federation-proxx-a1,federation-proxx-a2,federation-nginx}"
    : "${DEPLOY_PUBLIC_HOST:=staging-proxx.promethean.rest}"
    : "${DEPLOY_ENABLE_TLS:=false}"
    : "${DEPLOY_HEALTH_TIMEOUT_SECONDS:=240}"
    : "${DEPLOY_PRUNE_BEFORE_BUILD:=false}"
    : "${DEPLOY_ENV_APPEND:=FEDERATION_CLUSTER_ID=staging
FEDERATION_PUBLIC_HOST_SUFFIX=${DEPLOY_PUBLIC_HOST}
FEDERATION_DEFAULT_OWNER_SUBJECT=did:web:${DEPLOY_PUBLIC_HOST}
FEDERATION_NGINX_PUBLISHED_PORT=8890
PROXX_CONTAINER_UID=1000
PROXX_CONTAINER_GID=1000
PROXX_RUNTIME_DATA_DIR=./runtime-data-v3
PROXX_CONTAINER_HOME=/home/node
